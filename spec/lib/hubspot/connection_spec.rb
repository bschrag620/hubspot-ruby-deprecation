describe DeprecatedHubspot::Connection do
  before(:each) do
  ...
  describe '.get_json' do
    it 'delegates url format to DeprecatedHubspot::Utils, call HTTParty get and returns response' do
      @http_response.success? { true }
      @http_response.parsed_response { {} }
      @http_response.code { 200 }
      @http_response.body { 'mocked response' }

      mock(DeprecatedHubspot::Connection).generate_url(@url, {}) { @url }
      mock(DeprecatedHubspot::Connection).get(@url, format: :json, read_timeout: nil, open_timeout: nil) { @http_response }
      DeprecatedHubspot::Connection.get_json(@url, {})
    end
  end

  describe '.post_json' do
    it 'delegates url format to DeprecatedHubspot::Utils, call HTTParty post and returns response' do
      @http_response.success? { true }
      @http_response.parsed_response { {} }
      @http_response.code { 200 }
      @http_response.body { 'mocked response' }
      ...
      mock(DeprecatedHubspot::Connection).generate_url(@url, {}) { @url }
      mock(DeprecatedHubspot::Connection).post(@url, body: "{}", headers: {"Content-Type"=>"application/json"}, format: :json, read_timeout: nil, open_timeout: nil) { @http_response }
      DeprecatedHubspot::Connection.post_json(@url, params: {}, body: {})
    end
  end

  describe '.delete_json' do
    it 'delegates url format to DeprecatedHubspot::Utils, call HTTParty delete and returns response' do
      @http_response.success? { true }
      @http_response.code { 200 }
      @http_response.body { 'mocked response' }

      mock(DeprecatedHubspot::Connection).generate_url(@url, {}) { @url }
      mock(DeprecatedHubspot::Connection).delete(@url, format: :json, read_timeout: nil, open_timeout: nil) { @http_response }
      DeprecatedHubspot::Connection.delete_json(@url, {})
    end
  end
  ...
  context 'private methods' do
    describe ".generate_url" do
      ...
      let(:options){{}}
      subject{ DeprecatedHubspot::Connection.send(:generate_url, path, params, options) }
      before{ DeprecatedHubspot.configure(hapikey: "demo", portal_id: "62515") }
      ...
      context "when configure hasn't been called" do
        before{ DeprecatedHubspot::Config.reset! }
        it "raises a config exception" do
          expect{ subject }.to raise_error DeprecatedHubspot::ConfigurationError
        end
      end
      ...
      context "with interpolations but no params" do
        ...
        it "raises an interpolation exception" do
          expect{ subject }.to raise_error DeprecatedHubspot::MissingInterpolation
        end
      end
      ...
      context "passing Array as parameters for batch mode, key is prefixed with batch_" do
        let(:path) { DeprecatedHubspot::ContactList::LIST_BATCH_PATH }
