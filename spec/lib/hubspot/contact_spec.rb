RSpec.describe DeprecatedHubspot::Contact do
  ...
  describe '#initialize' do
    subject{ DeprecatedHubspot::Contact.new(example_contact_hash) }
    it{ should be_an_instance_of DeprecatedHubspot::Contact }
  ...
  describe '.create!' do
    ...
    let(:params){{}}
    subject{ DeprecatedHubspot::Contact.create!(email, params) }
    ...
    context "when the contact already exists" do
      it "updates the contact" do
        VCR.use_cassette("contacts/update_contact") do
          existing_contact = DeprecatedHubspot::Contact.create!("contact@example.com")
          email = existing_contact.email
          new_email = "new_email@example.com"
          params = { email: new_email }

          contact = DeprecatedHubspot::Contact.createOrUpdate(email, params)

          assert_requested :post, hubspot_api_url("/contacts/v1/contact/createOrUpdate/email/#{email}?hapikey=demo")
          expect(contact).to be_a(DeprecatedHubspot::Contact)
    ...
    context "with an invalid email" do
      it "raises an error" do
        VCR.use_cassette("contact_create_or_update_invalid_email") do
          expect {
            DeprecatedHubspot::Contact.createOrUpdate("not_an_email")
          }.to raise_error(DeprecatedHubspot::RequestError)
        end
      end
    end
  end
  ...
  describe '.create_or_update!' do
    ...
    let(:existing_contact) do
      DeprecatedHubspot::Contact.create!("morpheus@example.com", firstname: 'Morpheus')
    end
    before do
      DeprecatedHubspot::Contact.create_or_update!(params)
    end
    ...
    it 'creates and updates contacts' do
      contact = DeprecatedHubspot::Contact.find_by_id existing_contact.vid
      expect(contact.properties['firstname']).to eql 'Neo'
      latest_contact_email = DeprecatedHubspot::Contact.all(recent: true).first.email
      new_contact = DeprecatedHubspot::Contact.find_by_email(latest_contact_email)
  ...
  describe '.find_by_email' do
    context 'given an uniq email' do
      ...
      subject{ DeprecatedHubspot::Contact.find_by_email(email) }
      ...
      context 'when the contact cannot be found' do
        it 'raises an error' do
          expect { DeprecatedHubspot::Contact.find_by_email('notacontact@test.com') }.to raise_error(DeprecatedHubspot::RequestError)
        end
      end
    end
    ...
    context 'batch mode' do
      ...
      it 'find lists of contacts' do
        ...
        contacts = DeprecatedHubspot::Contact.find_by_email(emails)
        pending
      end
    end
  end
  ...
  describe '.find_by_id' do
    before do
      @s = StringIO.new
      DeprecatedHubspot::Config.logger = Logger.new(@s)
    end

    context 'given an uniq id' do
      cassette 'contact_find_by_id'
      subject{ DeprecatedHubspot::Contact.find_by_id(vid) }
      ...
      context 'when the contact cannot be found' do
        it 'raises an error' do
          expect { DeprecatedHubspot::Contact.find_by_id(9999999) }.to raise_error(DeprecatedHubspot::RequestError)
    ...
    context 'batch mode' do
      ...
      it 'find lists of contacts' do
        expect { DeprecatedHubspot::Contact.find_by_id([82325]) }.to raise_error(DeprecatedHubspot::ApiError)
  ...
  describe '.find_by_utk' do
    context 'given an uniq utk' do
      ...
      subject{ DeprecatedHubspot::Contact.find_by_utk(utk) }
      ...
      context 'when the contact cannot be found' do
        it 'raises an error' do
          expect { DeprecatedHubspot::Contact.find_by_utk('invalid') }.to raise_error(DeprecatedHubspot::RequestError)
        end
      end
    end
    ...
    context 'batch mode' do
      ...
      it 'find lists of contacts' do
        ...
        expect { DeprecatedHubspot::Contact.find_by_utk(utks) }.to raise_error(DeprecatedHubspot::ApiError)
      end
    end
  end

  describe '.search' do
    subject { DeprecatedHubspot::Contact.search(query, options) }
    ...
    context 'when query returns contacts' do
      ...
      it { subject['contacts'].each { |contact| expect(contact).to be_an_instance_of(DeprecatedHubspot::Contact) } }
  ...
  describe '.all' do
    context 'all contacts' do
      ...
      it 'must get the contacts list' do
        contacts = DeprecatedHubspot::Contact.all
      ...
      it 'must get the contacts list with paging data' do
        contact_data = DeprecatedHubspot::Contact.all({paged: true})
      ...
      it 'must filter only 2 contacts' do
        contacts = DeprecatedHubspot::Contact.all(count: 2)
        expect(contacts.size).to eql 2
      end

      it 'it must offset the contacts' do
        single_list = DeprecatedHubspot::Contact.all(count: 1)
        expect(single_list.size).to eql 1
        first = single_list.first

        second = DeprecatedHubspot::Contact.all(count: 1, vidOffset: first.vid).first
    ...
    context 'recent contacts' do
      ...
      it 'must get the contacts list' do
        contacts = DeprecatedHubspot::Contact.all(recent: true)
    ...
    context 'recent created contacts' do
      ...
      it 'must get the contacts list' do
        contacts = DeprecatedHubspot::Contact.all(recent_created: true)
        expect(contacts.size).to eql 20
  ...
  describe '.merge!' do
    ...
    let(:primary_contact) { DeprecatedHubspot::Contact.create!("primary_#{Time.now.to_i}@hsgem.com", primary_params) }
    let(:secondary_params) { { firstname: 'Wolverine' } }
    let(:secondary_contact) { DeprecatedHubspot::Contact.create!("secondary_#{Time.now.to_i}@hsgem.com", secondary_params) }

    subject { DeprecatedHubspot::Contact.merge!(primary_contact.vid, secondary_contact.vid) }

    it 'merges the contacts' do
      subject

      primary_find_by_id = DeprecatedHubspot::Contact.find_by_id primary_contact.vid
      primary_find_by_email = DeprecatedHubspot::Contact.find_by_email primary_contact.email
      secondary_find_by_id = DeprecatedHubspot::Contact.find_by_id secondary_contact.vid
      secondary_find_by_email = DeprecatedHubspot::Contact.find_by_email secondary_contact.email
  ...
  describe '#update!' do
    ...
    let(:contact){ DeprecatedHubspot::Contact.new(example_contact_hash) }
    ...
    context 'when the request is not successful' do
      let(:contact){ DeprecatedHubspot::Contact.new({'vid' => 'invalid', 'properties' => {}})}
      it 'raises an error' do
        expect{ subject }.to raise_error DeprecatedHubspot::RequestError
      end
    end
  ...
  describe '#destroy!' do
    ...
    let(:contact){ DeprecatedHubspot::Contact.create!("newcontact_y_#{Time.now.to_i}@hsgem.com") }
