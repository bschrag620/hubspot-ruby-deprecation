describe DeprecatedHubspot::Engagement do
  ...
  describe "#initialize" do
    subject{ DeprecatedHubspot::Engagement.new(example_engagement_hash) }
    it  { should be_an_instance_of DeprecatedHubspot::Engagement }
  ...
  describe 'EngagementNote' do
    describe ".create!" do
      ...
      body = "Test note"
      subject { DeprecatedHubspot::EngagementNote.create!(nil, body) }
    ...
    describe ".find" do
      ...
      let(:engagement) {DeprecatedHubspot::EngagementNote.new(example_engagement_hash)}

      it 'must find by the engagement id' do
        find_engagement = DeprecatedHubspot::EngagementNote.find(engagement.id)
    ...
    describe ".find_by_company" do
      ...
      let(:engagement) {DeprecatedHubspot::EngagementNote.new(example_associated_engagement_hash)}

      it 'must find by company id' do
        find_engagements = DeprecatedHubspot::EngagementNote.find_by_company(engagement.associations["companyIds"].first)
    ...
    describe ".find_by_contact" do
      ...
      it 'must find by contact id' do
        find_engagements = DeprecatedHubspot::EngagementNote.find_by_contact(engagement.associations["contactIds"].first)
        find_engagements.should_not be_nil
        find_engagements.any?{|engagement| engagement.id == engagement.id and engagement.body == engagement.body}.should be_true
      end
    end
    ...
    describe ".find_by_association" do
      ...
      it 'must raise for fake association type' do
        expect {
          DeprecatedHubspot::EngagementNote.find_by_association(1, 'FAKE_TYPE')
        }.to raise_error
      end
    end
    ...
    describe ".associate!" do
      ...
      let(:engagement) { DeprecatedHubspot::EngagementNote.create!(nil, 'note') }
      let(:contact) { DeprecatedHubspot::Contact.create!("newcontact#{Time.now.to_i}@hsgem.com") }
      subject { DeprecatedHubspot::Engagement.associate!(engagement.id, 'contact', contact.vid) }

      it 'associate an engagement to a resource' do
        subject
        found_by_contact = DeprecatedHubspot::Engagement.find_by_contact(contact.vid)
    ...
    describe '#destroy!' do
      ...
      let(:engagement) {DeprecatedHubspot::EngagementNote.create!(nil, 'test note') }

      it 'should remove from hubspot' do
        expect(DeprecatedHubspot::Engagement.find(engagement.id)).to_not be_nil

        expect(engagement.destroy!).to be_true
        expect(engagement.destroyed?).to be_true

        expect(DeprecatedHubspot::Engagement.find(engagement.id)).to be_nil
      end
    end
  end
  ...
  describe 'EngagementCall' do
    ...
    describe ".create!" do
      ...
      body = "Test call"
      subject { DeprecatedHubspot::EngagementCall.create!(nil, body, 0) }
      its(:id) { should_not be_nil }
      its(:body) { should eql body }
    end
    ...
    describe '#destroy!' do
      ...
      let(:engagement) { DeprecatedHubspot::EngagementCall.create!(nil, 'test call', 0) }
