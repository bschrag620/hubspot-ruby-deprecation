describe DeprecatedHubspot::Deal do
  let(:portal_id) { 62515 }
  ...
  describe "#initialize" do
    subject{ DeprecatedHubspot::Deal.new(example_deal_hash) }
    it  { should be_an_instance_of DeprecatedHubspot::Deal }
  ...
  describe ".create!" do
    ...
    subject { DeprecatedHubspot::Deal.create!(portal_id, [company_id], [vid], {}) }
  ...
  describe ".find" do
    cassette "deal_find"
    let(:deal) {DeprecatedHubspot::Deal.create!(portal_id, [company_id], [vid], { amount: amount})}

    it 'must find by the deal id' do
      find_deal = DeprecatedHubspot::Deal.find(deal.deal_id)
  ...
  describe '.find_by_company' do
    ...
    let(:company) { DeprecatedHubspot::Company.create!('Test Company') }
    let(:deal) { DeprecatedHubspot::Deal.create!(portal_id, [company.vid], [vid], { amount: amount }) }

    it 'returns company deals' do
      deals = DeprecatedHubspot::Deal.find_by_company(company)
  ...
  describe '.recent' do
    ...
    it 'must get the recents updated deals' do
      deals = DeprecatedHubspot::Deal.recent

      first = deals.first
      last = deals.last

      expect(first).to be_a DeprecatedHubspot::Deal
      ...
      expect(last).to be_a DeprecatedHubspot::Deal
    ...
    it 'must filter only 2 deals' do
      deals = DeprecatedHubspot::Deal.recent(count: 2)
      expect(deals.size).to eql 2
    end

    it 'it must offset the deals' do
      deal = DeprecatedHubspot::Deal.recent(count: 1, offset: 1).first
  ...
  describe '#destroy!' do
    ...
    it 'should remove from hubspot' do
      pending
      expect(DeprecatedHubspot::Deal.find(deal.deal_id)).to_not be_nil

      expect(deal.destroy!).to be_true
      expect(deal.destroyed?).to be_true

      expect(DeprecatedHubspot::Deal.find(deal.deal_id)).to be_nil
    end
  end
