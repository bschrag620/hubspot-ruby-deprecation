module DeprecatedHubspot
  ...
  class Blog
    ...
    class << self
      ...
      # @return [DeprecatedHubspot::Blog] the first 20 blogs or empty_array
      def list
        response = DeprecatedHubspot::Connection.get_json(BLOG_LIST_PATH, {})
      ...
      # @return DeprecatedHubspot::Blog
      def find_by_id(id)
        response = DeprecatedHubspot::Connection.get_json(GET_BLOG_BY_ID_PATH, { blog_id: id })
        new(response)
      end
    end
    ...
    # @return [DeprecatedHubspot::BlogPost]
    def posts(params = {})
      default_params = {
      ...
      raise DeprecatedHubspot::InvalidParams.new('params must be passed as a hash') unless params.is_a?(Hash)
      params = default_params.merge(params)

      raise DeprecatedHubspot::InvalidParams.new('State parameter was invalid') unless [false, 'PUBLISHED', 'DRAFT'].include?(params[:state])
      params.each { |k, v| params.delete(k) if v == false }

      response = DeprecatedHubspot::Connection.get_json(BLOG_POSTS_PATH, params)
  ...
  class BlogPost
    ...
    def self.find_by_blog_post_id(id)
      response = DeprecatedHubspot::Connection.get_json(GET_BLOG_POST_BY_ID_PATH, { blog_post_id: id })
      new(response)
    end
      ...
      @topics ||= begin
        if @properties['topic_ids'].empty?
          ...
          @properties['topic_ids'].map do |topic_id|
            DeprecatedHubspot::Topic.find_by_topic_id(topic_id)
          end
        end
      end
    end
