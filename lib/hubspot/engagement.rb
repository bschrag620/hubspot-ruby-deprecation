...
module DeprecatedHubspot
  ...
  class Engagement
    ...
    class << self
      def create!(params={})
        response = DeprecatedHubspot::Connection.post_json(CREATE_ENGAGMEMENT_PATH, params: {}, body: params )
        new(HashWithIndifferentAccess.new(response))
      end

      def find(engagement_id)
        begin
          response = DeprecatedHubspot::Connection.get_json(ENGAGEMENT_PATH, { engagement_id: engagement_id })
          response ? new(HashWithIndifferentAccess.new(response)) : nil
        rescue DeprecatedHubspot::RequestError => ex
      ...
      def find_by_association(association_id, association_type)
        ...
        raise DeprecatedHubspot::InvalidParams, 'expecting Integer parameter' unless association_id.try(:is_a?, Integer)
        raise DeprecatedHubspot::InvalidParams, 'expecting String parameter' unless association_type.try(:is_a?, String)
      ...
      def associate!(engagement_id, object_type, object_vid)
        DeprecatedHubspot::Connection.put_json(ASSOCIATE_ENGAGEMENT_PATH,
                                     params: {
    ...
    def destroy!
      DeprecatedHubspot::Connection.delete_json(ENGAGEMENT_PATH, {engagement_id: id})
      @destroyed = true
    end
    ...
    # @return [DeprecatedHubspot::Engagement] self
    def update!(params)
      data = {
      ...
      DeprecatedHubspot::Connection.put_json(ENGAGEMENT_PATH, params: { engagement_id: id }, body: data)
      self
    end
  end
