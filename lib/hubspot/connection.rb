  ...
  class Connection
    ...
    class << self
      def get_json(path, opts)
        ...
        raise(DeprecatedHubspot::RequestError.new(response)) unless response.success?
      ...
      def read_timeout(opts = {})
        opts.delete(:read_timeout) || DeprecatedHubspot::Config.read_timeout
      end

      def open_timeout(opts = {})
        opts.delete(:open_timeout) || DeprecatedHubspot::Config.open_timeout
      end
      ...
      def handle_response(response)
        if response.success?
          ...
          raise(DeprecatedHubspot::RequestError.new(response))
        end
      end

      def log_request_and_response(uri, response, body=nil)
        DeprecatedHubspot::Config.logger.info "DeprecatedHubspot: #{uri}.\nBody: #{body}.\nResponse: #{response.code} #{response.body}"
      end

      def generate_url(path, params={}, options={})
        if DeprecatedHubspot::Config.access_token.present?
          options[:hapikey] = false
        else
          DeprecatedHubspot::Config.ensure! :hapikey
        end
        path = path.clone
        params = params.clone
        base_url = options[:base_url] || DeprecatedHubspot::Config.base_url
        params["hapikey"] = DeprecatedHubspot::Config.hapikey unless options[:hapikey] == false

        if path =~ /:portal_id/
          DeprecatedHubspot::Config.ensure! :portal_id
          params["portal_id"] = DeprecatedHubspot::Config.portal_id if path =~ /:portal_id/
        end
        ...
        raise(DeprecatedHubspot::MissingInterpolation.new("Interpolation not resolved")) if path =~ /:/
