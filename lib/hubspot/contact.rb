  ...
  class Contact
    ...
    class << self
      ...
      def create!(email, params={})
        ...
        post_data = {properties: DeprecatedHubspot::Utils.hash_to_properties(params_with_email)}
        response = DeprecatedHubspot::Connection.post_json(CREATE_CONTACT_PATH, params: {}, body: post_data )
        new(response)
      end
      ...
      def all(opts={})
        ...
        path, opts =
        if recent_created
          [RECENTLY_CREATED_PATH, DeprecatedHubspot::ContactProperties.add_default_parameters(opts)]
        elsif recent
          [RECENTLY_UPDATED_PATH, DeprecatedHubspot::ContactProperties.add_default_parameters(opts)]
        else
          [CONTACTS_PATH, opts]
        end
      ...
      def createOrUpdate(email, params={})
        post_data = {properties: DeprecatedHubspot::Utils.hash_to_properties(params.stringify_keys)}
        response = DeprecatedHubspot::Connection.post_json(CREATE_OR_UPDATE_PATH, params: { contact_email: email }, body: post_data )
        ...
        query = contacts.map do |ch|
          ...
          if contact_hash[:vid]
            contact_param = {
              ...
              properties: DeprecatedHubspot::Utils.hash_to_properties(contact_hash.except(:vid))
            }
          elsif contact_hash[:email]
            contact_param = {
              email: contact_hash[:email],
              properties: DeprecatedHubspot::Utils.hash_to_properties(contact_hash.except(:email))
            }
          else
            raise DeprecatedHubspot::InvalidParams, 'expecting vid or email for contact'
          end
          contact_param
        end
        DeprecatedHubspot::Connection.post_json(BATCH_CREATE_OR_UPDATE_PATH,
                                      params: {},
                                      body: query)
      end
      ...
      def find_by_id(vids)
        ...
        else raise DeprecatedHubspot::InvalidParams, 'expecting Integer or Array of Integers parameter'
        end

        response = DeprecatedHubspot::Connection.get_json(path, params)
        raise DeprecatedHubspot::ApiError if batch_mode
        new(response)
      end
      ...
      def find_by_email(emails)
        ...
        else raise DeprecatedHubspot::InvalidParams, 'expecting String or Array of Strings parameter'
        end
      ...
      def search(query, options = {})
        ...
        response = DeprecatedHubspot::Connection.get_json(QUERY_PATH, { q: query, count: count, offset: offset })
      ...
      def merge!(primary_contact_vid, secondary_contact_vid)
        DeprecatedHubspot::Connection.post_json(
          MERGE_CONTACT_PATH,
    ...
    # @return [DeprecatedHubspot::Contact] self
    def update!(params)
      query = {"properties" => DeprecatedHubspot::Utils.hash_to_properties(params.stringify_keys!)}
      DeprecatedHubspot::Connection.post_json(UPDATE_CONTACT_PATH, params: { contact_id: vid }, body: query)
    ...
    def destroy!
      DeprecatedHubspot::Connection.delete_json(DESTROY_CONTACT_PATH, { contact_id: vid })
      @destroyed = true
    end
