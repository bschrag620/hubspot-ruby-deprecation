  ...
  class DealPipeline
    ...
    class << self
      def find(pipeline_id)
        response = DeprecatedHubspot::Connection.get_json(PIPELINE_PATH, { pipeline_id: pipeline_id })
        new(response)
      end

      def all
        response = DeprecatedHubspot::Connection.get_json(PIPELINES_PATH, {})
        response.map { |p| new(p) }
      end

      # Creates a DealPipeline
      # {https://developers.hubspot.com/docs/methods/deal-pipelines/create-deal-pipeline}
      # @return [DeprecatedHubspot::PipeLine] Company record
      def create!(post_data={})
        response = DeprecatedHubspot::Connection.post_json(PIPELINES_PATH, params: {}, body: post_data)
        new(response)
      end
    end
    ...
    def destroy!
      DeprecatedHubspot::Connection.delete_json(PIPELINE_PATH, pipeline_id: @pipeline_id)
    end
